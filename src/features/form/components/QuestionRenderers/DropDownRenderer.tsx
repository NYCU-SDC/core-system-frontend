...
export const DropdownRenderer = ({ question, value, onAnswerChange }: { question: FormsQuestionResponse; value: string; onAnswerChange: (questionId: string, value: string) => void }) => {
	...
	return <Select options={choices.map(choice => ({ value: choice.id, label: choice.name ?? choice.id }))} value={value || undefined} onValueChange={newValue => onAnswerChange(question.id, newValue)} />;
};
