...
export function RadioCard({ options, themeColor, ...props }: RadioCardProps) {
  return (
    <RadioGroup.Root className={styles.radioGroup} {...props}>
      {options.map((option) => {
        const cardStyle: CSSProperties = themeColor
          ? { borderColor: themeColor, ['--indicator-color' as any]: themeColor }
          : {};
