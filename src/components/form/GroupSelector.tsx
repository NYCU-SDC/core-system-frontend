...
export function GroupSelector({ selectedGroups, availableGroups, onGroupsChange, label = "Units", placeholder = "Select groups" }: GroupSelectorProps) {
	const [isOpen, setIsOpen] = useState(false);

	const toggleGroup = (group: string) => {
		const newGroups = selectedGroups.includes(group) ? selectedGroups.filter(g => g !== group) : [...selectedGroups, group];
	...
	return (
		<div className="relative">
			...
			>
				<span className={`text-sm ${selectedGroups.length === 0 ? "text-gray-500" : "text-gray-900"}`}>{getDisplayText()}</span>
				<svg
					...
					<div className="absolute top-full left-0 right-0 z-[10] mt-1 bg-white border border-slate-300 rounded-md shadow-lg">
						<ScrollArea className="p-2 max-h-[120px]">
							{availableGroups.map(group => (
								<div
									key={group}
									className="flex items-center gap-2 py-1 hover:bg-gray-100 px-2 rounded accent-slate-800"
								>
									<input
										type="checkbox"
										id={`group-${group}`}
